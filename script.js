...
downloadPowerPointButton.addEventListener("click", function () {
  ...
  // Adding text boxes to the slide with the values of the certificate fields
  var textboxes = [];
  var dataIndex = [];
  Inputs.querySelectorAll(".certinputs").forEach(function (input) {
    if (titles.includes(input.value)) {
      input.dataset.editable = "1";
      textboxes.push(slide.addText(input.value, { x: 0.5, y: 0.5, w: 6, h: 1.5 }));
      dataIndex.push(titles.indexOf(input.value));
    }
  });

  // Creating the PowerPoint file with the certificate fields as editable text boxes
  var pptxData = pptx.write("blob");
  
  // Creating Download Link
  var link = document.createElement("a");
  link.download = "certificate.pptx";
  link.href = URL.createObjectURL(pptxData);
  link.click();
  
  // Removing the editable attribute from the certificate fields
  textboxes.forEach(function (textbox, j) {
    textbox.options.placeholder = titles[dataIndex[j]];
    textbox.options.editable = true;
  });
});
